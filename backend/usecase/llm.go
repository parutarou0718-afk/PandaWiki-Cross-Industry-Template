...
type LLMUsecase struct {
	...
	promptRepo       PromptStore
	edition          EditionReader
...
type PromptStore interface {
	GetPromptContent(ctx context.Context, kbID string) (string, error)
	GetSummaryPrompt(ctx context.Context, kbID string) (string, error)
}

type EditionReader interface {
	Get(ctx context.Context) (*domain.EditionConfig, error)
}

func NewLLMUsecase(config *config.Config, rag rag.RAGService, conversationRepo *pg.ConversationRepository, kbRepo *pg.KnowledgeBaseRepository, nodeRepo *pg.NodeRepository, modelRepo *pg.ModelRepository, promptRepo PromptStore, edition EditionReader, logger *log.Logger) *LLMUsecase {
...
func (u *LLMUsecase) ResolveChatPrompt(ctx context.Context, kbID string) string {
	if prompt, err := u.promptRepo.GetPromptContent(ctx, kbID); err == nil && strings.TrimSpace(prompt) != "" {
		return prompt
	} else if err != nil {
		u.logger.Warn("get knowledge base chat prompt failed; using system default", log.Error(err))
		return domain.SystemDefaultPrompt
	}
	return u.resolveEditionPrompt(ctx, func(prompts domain.EditionPrompts) string { return prompts.Chat }, domain.SystemDefaultPrompt)
}

func (u *LLMUsecase) ResolveSummaryPrompt(ctx context.Context, kbID string) string {
	if prompt, err := u.promptRepo.GetSummaryPrompt(ctx, kbID); err == nil && strings.TrimSpace(prompt) != "" {
		return prompt
	} else if err != nil {
		u.logger.Warn("get knowledge base summary prompt failed; using system default", log.Error(err))
		return domain.SystemDefaultSummaryPrompt
	}
	return u.resolveEditionPrompt(ctx, func(prompts domain.EditionPrompts) string { return prompts.Summary }, domain.SystemDefaultSummaryPrompt)
}

func (u *LLMUsecase) resolveEditionPrompt(ctx context.Context, selectPrompt func(domain.EditionPrompts) string, fallback string) string {
	config, err := u.edition.Get(ctx)
	if err != nil {
		u.logger.Warn("get edition prompt failed; using system default", log.Error(err))
		return fallback
	}
	if prompt := strings.TrimSpace(selectPrompt(config.DefaultPrompts)); prompt != "" {
		return prompt
	}
	return fallback
}
...
func (u *LLMUsecase) BuildConversationMessageWithRAG(
	...
	if len(msgs) > 0 {
		...
		if len(historyMessages) > 0 {
			...
			if systemPrompt == "" {
				systemPrompt = u.ResolveChatPrompt(ctx, kbID)
			}
...
// GenerateWithConfiguredChatModel keeps model-kit wiring inside LLMUsecase so
// callers cannot bypass the application's configured chat-model path.
func (u *LLMUsecase) GenerateWithConfiguredChatModel(ctx context.Context, configured *domain.Model, messages []*schema.Message) (string, error) {
	modelkitModel, err := configured.ToModelkitModel()
	if err != nil {
		return "", err
	}
	...
	return u.Generate(ctx, chatModel, messages)
}
...
func (u *LLMUsecase) requestSummary(ctx context.Context, kbID string, chatModel model.BaseChatModel, name, content string) (string, error) {
	summaryPrompt := u.ResolveSummaryPrompt(ctx, kbID)
