...
func createApp() (*App, error) {
	...
	editionUsecase := usecase.NewEditionUsecase(systemSettingRepo, logger)
	llmUsecase := usecase.NewLLMUsecase(configConfig, ragService, conversationRepository, knowledgeBaseRepository, nodeRepository, modelRepository, promptRepo, editionUsecase, logger)
	...
	editionHandler := v1.NewEditionHandler(echo, baseHandler, logger, authMiddleware, editionUsecase)
	reportProfileRepo := pg2.NewReportProfileRepo(db)
	reportRepo := pg2.NewReportRepo(db)
	reportUsecase := usecase.NewReportUsecase(reportProfileRepo, reportRepo, authRepo, knowledgeBaseRepository, llmUsecase, modelUsecase, editionUsecase, logger)
	...
	shareAppHandler := share.NewShareAppHandler(echo, baseHandler, logger, appUsecase, editionUsecase)
