...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const icon = getImagePath(
    edition?.branding?.logo || kbDetail?.settings?.icon || '',
    basePath,
  );
  return {
    metadataBase: new URL(process.env.TARGET || ''),
    title: edition?.product_name || kbDetail?.settings?.title || 'Panda-Wiki',
    description: edition?.home_description || kbDetail?.settings?.desc || '',
