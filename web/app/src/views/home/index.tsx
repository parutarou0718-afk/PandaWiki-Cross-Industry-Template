...
const handleBannerProps = (
  config: any = {},
  basePath: string,
  edition?: { product_name?: string; home_description?: string },
) => {
  return {
    title: {
      text: config.title || edition?.product_name,
    },
    subtitle: {
      text: config.subtitle || edition?.home_description,
    },
...
const Welcome = () => {
  ...
  const handleComponentProps = (data: any) => {
    ...
    switch (data.type) {
      ...
      case 'banner':
        return {
          ...handleBannerProps(config, basePath, edition),
