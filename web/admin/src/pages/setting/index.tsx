...
import ReportProfiles from '../report-profiles';
...
const SettingTabs: { label: string; id: string }[] = [
  ...
  { label: 'MCP 设置', id: 'mcp' },
  { label: '行业版本', id: 'edition' },
  { label: '报告模板', id: 'report-profiles' },
];
    ...
    >
      <Card sx={{ mb: 2, display: 'flex', justifyContent: 'center' }}>
        ...
        >
          {SettingTabs.filter(
            tab =>
              (tab.id !== 'edition' && tab.id !== 'report-profiles') ||
      ...
      >
        ...
        {activeTab === 'report-profiles' && <ReportProfiles />}
      </Card>
    </Box>
  );
};
